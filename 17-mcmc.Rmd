...
This distribution is depicted in green in the previous plot.
...
The proposals and acceptance probability only depend on the current state, and not past states (given the current state).
...
Suppose we wish to estimate $\theta$, the proportion of Cal Poly students who have read a non-school related book in 2022.
...
1. See below. The Normal distribution proposal can propose values outside of (0, 1), so we set $\pi(\theta|y=4)$ equal to 0 for $\theta\notin(0,1)$.
This way, proposals to states outside (0, 1) will never be accepted.
...
1. The theoretical posterior distribution is the Beta(5, 24) distribution, depicted in green above.
...
1. Write code to run the algorithm and plot the simulated values of $\theta$.
1. Write and run JAGS code to approximate the posterior distribution, and compare with the previous part.
...
1. The code and output is below.
Now the state is two-dimensional, $(\mu, p)$, and the trace plot shows how the Markov chain explores this two-dimensional space as it steps.
You can't tell from this trace plot when the Markov chain rejects a proposal and stays in the same place since the plot points get overlaid in that case, but you can see where the step numbers coincide.
...
The results are similar, but not quite the same as our code from scratch in the previous part.
JAGS has a lot of built in features that improve efficiency.
In particular, JAGS is making smarter proposals and is not rejecting as many proposals as our from-scratch algorithm.
The scatterplots of simulated $(\mu, p)$ pairs kind of show this; the plot based on the from-scratch algorithm is "thinner" than the one based on JAGS because the from-scratch algorithm rejects proposals and sits in place more often.
