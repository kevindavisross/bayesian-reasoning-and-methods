...
1. $\sigma$ measures how much individual body temperatures vary from person to person.
1. About 68% of body temperatures are within 1 degree F of the mean, 95% within 2 degrees F of the mean, and 99.7% within 3 degrees F of the mean.
...
plot(theta, prior, type='l', xlim=range(theta), ylim=ylim, col="skyblue", xlab='theta', ylab='')
par(new=T)
plot(theta, likelihood/sum(likelihood), type='l', xlim=range(theta), ylim=ylim, col="orange", xlab='', ylab='')
par(new=T)
...
Posterior is compromise between prior and likelihood.
...
- The prior has very little influence on the posterior
- The posterior distribution is approximately Normal with posterior mean `r round(posterior_mean, 2)` and posterior SD `r round(posterior_sd, 2)`.
- The small posterior SD concentrates plausibility around a fairly narrow ranges of values of $\theta$.
- There is posterior probability of 98% that population mean healthy human body temperature is between `r round(theta[max(which(cumsum(posterior) < 0.01))], 2)` and `r round(theta[max(which(cumsum(posterior) < 0.99))], 2)` degrees F.
...
- There is posterior probability of 98% that population mean healthy human body temperature is between `r round(theta[max(which(cumsum(posterior) < 0.025))], 2)` and `r round(theta[max(which(cumsum(posterior) < 0.975))], 2)` degrees F.
- It is 19 times more plausible that population mean healthy human body temperature is inside the interval  (`r round(theta[max(which(cumsum(posterior) < 0.025))], 2)`, `r round(theta[max(which(cumsum(posterior) < 0.975))], 2)`) degrees F than outside.
...
- We estimate with 95% confidence that population mean healthy human body temperature is between `r round(y - 2 * sigma / sqrt(n), 2)` and `r round(y + 2 * sigma / sqrt(n), 2)` degrees F.
- If many samples were selected and a 95% confidence interval computed based on each one, then 95% of the confidence intervals would contain the true population mean human body temperature.
...
Bayesian interval provides a probability statement about the parameter.
Frequentist interval is based on what would happen over many samples.
...
- There is posterior probability of 99.999% that population mean healthy human body temperature is less than 98.0 degrees F.
- We'd be willing to be a very large amount of money on it.
...
- A sample mean of `r y` is `r -round((y - 98)/(sigma / sqrt(n)), 2)` SDs below the hypothesized value.
- If population mean human body temperature is 98.0 then in less than 0.01% of samples the sample mean body temperature will be as extreme as 97.7.
- With a p-value less than 0.0001 there is strong evidence to reject the null hypothesis and conclude that population mean healthy human body temperature is less than 98.0
...
The p-value is similar in magnitude to the posterior probability that the null hypothesis is true.

Bayesian can compute a probability that a hypothesis is true.
p-value is confusing.
