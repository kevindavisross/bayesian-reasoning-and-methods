...
More plausibility shifted towards $\theta$ values that are close to the observed sample proportion.
Posterior has smaller SD than the prior.
...
theta_post = theta_sim[y_sim == y_obs]

hist(theta_post)

sum(theta_post > 0.75) / length(theta_post)

```

Plausibility has shifted towards the observed sample proportion.
Smaller range of plausible values compared to the prior.
...
The likelihood is small outside of a range of values near the observed sample proportion.
The small likelihood "zeros out" the posterior probability for values of $\theta$ away from the sample proportion, even for values with a not small prior probability.
