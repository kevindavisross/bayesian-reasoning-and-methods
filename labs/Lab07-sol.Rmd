...
hist(theta_ratio,
     freq = FALSE,
     xlab = "Ratio of survival probabilities (no band / band)",
     main = "Prior Distribution")

quantile(theta_ratio, c(0.01, 0.99))

```

This prior distribution gives credibility to a wide range of scenarios: survival could be more likely either with or without the band, and the difference in survival probabilities could either be 0, small, or large.
Our 98% prior credible interval says probability of surviving can be anywhere from `r round(quantile(theta_ratio, c(0.01)), 2)` times as likely to `r round(quantile(theta_ratio, c(0.99)), 2)` times more likely for without bands than with.
...
```{r}
sum(theta_diff > 0) / length(theta_diff)
```

There is posterior probability of `r round(100 * sum(theta_diff > 0) / length(theta_diff), 1)` percent that penguins without bands are more likely to survive than those with bands. 
...
- There is posterior probability of 50% that the probability of surviving is between `r round(quantile(theta_diff, c(0.25)), 2)` and `r round(quantile(theta_diff, c(0.75)), 2)` greater for penguins without bands than with bands. It is equally plausible that the difference in survival probabilities is inside this interval than outside.
- There is posterior probability of 80% that the probability of surviving is between `r round(quantile(theta_diff, c(0.10)), 2)` and `r round(quantile(theta_diff, c(0.90)), 2)` greater for penguins without bands than with bands. It is four times more plausible that the difference in survival probabilities is inside this interval than outside.
- There is posterior probability of 98% that the probability of surviving is between `r round(quantile(theta_diff, c(0.01)), 2)` and `r round(quantile(theta_diff, c(0.99)), 2)` greater for penguins without bands than with bands. It is 49 times more plausible that the difference in survival probabilities is inside this interval than outside.
...
hist(theta_ratio,
     freq = FALSE,
     xlab = "Ratio survival probabilities (no band / band)",
     main = "Posterior Distribution")
...
```{r}
quantile(theta_ratio, c(0.01, 0.10, 0.25, 0.75, 0.90, 0.99))
```

- There is posterior probability of 50% that the probability of surviving is between `r round(quantile(theta_ratio, c(0.25)), 2)` and `r round(quantile(theta_ratio, c(0.75)), 2)` *times* greater for penguins without bands than with bands. It is equally plausible that the ratio of survival probabilities is inside this interval than outside.
- There is posterior probability of 80% that the probability of surviving is between `r round(quantile(theta_ratio, c(0.10)), 2)` and `r round(quantile(theta_ratio, c(0.90)), 2)` *times* greater for penguins without bands than with bands. It is four times more plausible that the ratio of survival probabilities is inside this interval than outside.
- There is posterior probability of 98% that the probability of surviving is between `r round(quantile(theta_ratio, c(0.01)), 2)` and `r round(quantile(theta_ratio, c(0.99)), 2)` *times* greater for penguins without bands than with bands. It is four times more plausible that the ratio of survival probabilities is inside this interval than outside.
...
theta_diff = sim_prior$theta1 - sim_prior$theta2
...
The prior distribution still gives credibility to a fairly wide range of values, but not as wide as for the independent prior.
The credible intervals based on the dependent prior are all narrower than those based on the dependent prior, reflecting a stronger prior belief towards "no difference".
...
theta_ratio = sim_prior$theta1 / sim_prior$theta2
...
Similar comments as for the difference.
...
Dependent prior represents a stronger prior belief in "no difference".
...
- Simulate a pair $(\theta_1, \theta_2)$ from the joint prior distribution.
- Given $\theta_1$, simulate $y_1$ from a Binomial(50, $\theta_1$) distribution
- Given $\theta_2$, simulate $y_2$ from a Binomial(50, $\theta_2$) distribution
- Repeat many times to generate many repetitions of $(\theta_1, \theta_2, y_1, y_2)$.
- Discard the repetitions for which either $y_1$ is not 31 or $y_2$ is not 16.
- Summarize the $(\theta_1, \theta_2)$ pairs for the remaining repetitions to approximate the joint posterior distribution.
- Among the remaining repetitions, count the repetitions for $\theta_1>\theta_$ and divide by the number of non-discarded repetitions to approximate the posterior probability that $\theta_1$ is greater than $\theta_2$
...
```{r}
sum(sim_posterior$theta1 > sim_posterior$theta2) / nrow(sim_posterior)
```

Pretty close to 100% credibility that penguins are more likely to survive without the band than with.
...
```{r}
theta_diff = sim_posterior$theta1 - sim_posterior$theta2

hist(theta_diff,
     freq = FALSE,
     xlab = "Difference of survival probabilities (no band - band)",
     main = "Posterior Distribution")

```


Pretty similar to the posterior based on the independent prior, but with a little less credibility on the largest differences.
...
```{r}
theta_ratio = sim_posterior$theta1 / sim_posterior$theta2
...
Pretty similar to the posterior based on the independent prior.
...
- Yes, there is a high posterior probability that penguins with metal bands are more likely to die than those without.
- Yes, because the penguins were randomly assigned to have a band or not, we can conclude that the metal bands are the cause of the difference in survival probabilities.
