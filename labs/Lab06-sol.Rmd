...
The likelihood is the Exponential($\theta$) density evaluated at $y=1.6$, computed for each value of $\theta$.
\[
...
By the Gamma property of cumulative times, the total time until 2 earthquakes follows a Gamma distribution with shape parameter 2 and rate parameter $\theta$.
...
For example, the likelihood for $\theta=0.25$ is $\frac{0.25^2}{(2-1)!}{4.8}^{2-1}e^{-4.8(0.25)} = 0.09$.
The likelihood is not the same as in the previous part because there are more samples of size two that yield a total wait time of 4.8 hours than those that yield a pair of wait times of 3.2 and 1.6.
However, the likelihoods are *proportionally* the same.
For example, the likelihood for $\theta=1.00$ is about 2.12 times greater than the likelihood for $\theta = 1.25$ in both this part and the previous part.
...
The total wait time for 100 earthquakes follows a *Gamma* distribution with shape parameter 100 and rate parameter $\theta$.
The likelihood is the Gamma(100, $\theta$) density evaluated at a value of 63.09 (for the wait time for the 100 earthquakes) computed for each $\theta$.
\begin{align*}
f(\bar{y}=63.09 / 100|\theta) & = \frac{\theta^{100}}{(100-1)!}{63.09}^{100-1}e^{-63.09\theta}\\
& \propto \theta^{100} e^{-63.09\theta}
\end{align*}

The sample mean time between earthquakes is 63.09/100 = 0.63 hours (about 38 minutes).
The likelihood is centered at the sample *rate* of 100/63.09 =  1.59 earthquakes per hour.
The posterior distribution follows the likelihood fairly closely.
...
The likelihood is the product of the likelihoods of $y=3.2$ and $y=1.6$.
\[
...
abline(v = qgamma(c(0.01, 0.99), alpha + y, lambda + n * y),
       col = "seagreen", lty = 2)
```
...
- The prior Gamma(4, 3) distribution reflects a "prior count" of 4 "earthquakes" in a "prior observation time" of 3 "hours", for an average rate of 4/3 = 1.33 earthquakes per hour.
- The sample reflects an observed count of 100 earthquakes in a total observation time of 63.09 hours, for an observed rate of 1.59 earthquakes per hour.
- The posterior reflects a total count of 104 earthquakes observed in a total time of 66.09 hours, for a posterior mean rate of 1.57 earthquakes per hour.

|              | Prior |  Data | Posterior |
|--------------|------:|------:|----------:|
| Total count  |     4 |   100 |       104 |
| Total time   |     3 | 63.09 |     66.09 |
| Average rate |  1.33 |  1.59 |      1.57 |

Therefore, the posterior distribution of $\theta$ is the Gamma distribution with shape parameter 104 and rate parameter 66.09.


Once you've derived the Gamma-Exponential theory, you can just use the theory.
But here are the details of deriving the posterior distribution in this example.
...
The results are similar to grid approximation.
...
The posterior distribution is the Gamma density with shape parameter $\alpha=104$ and rate parameter $\lambda = 66.09$. 
\begin{align*}
...
```{r}
qgamma(c(0.01, 0.99), alpha + n, lambda + n * y)
```
...
There is a posterior probability of 98% that the rate at which earthquakes occur in Southern California is between `r round(qgamma(c(0.01), alpha + n, lambda + n), 2)` and `r round(qgamma(c(0.99), alpha + n, lambda + n), 2)` earthquakes per hour.
Based on our prior beliefs and the observed data, it is 49 times more plausible that the per hour rate at which earthquakes occurs in Southern California lies inside this interval than outside.
...
The prior mean of the rate parameter is 4/3=1.333, based on a "prior observation time" of 3 "hours".
...
Conduct the simulation and find a central 98% posterior credible interval for the *mean* time between earthquakes.
...
Since $\theta$ is the rate, the mean waiting time is $1/\theta$.
Therefore, we can just simulate a value of $\theta$ from its posterior distribution, find $1/\theta$ and repeat many times.
...
quantile(1 / theta_sim, c(0.01, 0.99))
```


There is a 98% posterior probability that the mean time between earthquakes is between `r round(quantile(1 / theta_sim, c(0.01)), 2)` and `r round(quantile(1 / theta_sim, c(0.99)), 2)` hours (about `r round(60 * quantile(1 / theta_sim, c(0.01)), 2)` to `r round(60 * quantile(1 / theta_sim, c(0.99)), 2)` minutes.)
...
Conduct the simulation and find a central 98% posterior credible interval for the *median* time between earthquakes.
...
In general, finding the posterior distribution of the median could be tricky.
But for Exponential, we have that the median is $\log(2)/\theta$.
Therefore, we can just simulate a value of $\theta$ from its posterior distribution, find $\log(2)/\theta$ and repeat many times.

(We could have just used the simulated means from the previous part, but I'm re-simulating and including the whole code for completeness.)
...
There is a 98% posterior probability that the *median* time between earthquakes is between `r round(quantile(log(2) / theta_sim, c(0.01)), 2)` and `r round(quantile(log(2) / theta_sim, c(0.99)), 2)` hours (about `r round(60 * quantile(log(2) / theta_sim, c(0.01)), 2)` to `r round(60 * quantile(log(2) / theta_sim, c(0.99)), 2)` minutes.)
...
- Simulate a value of $\theta$ from its Gamma(104, 66.09) posterior distribution
- Given $\theta$ simulate a value of $Y$ from an Exponential distribution with rate $\theta$
- Repeat many times and summarizes the $Y$'s.
...
There is a posterior predictive probability of 95% that the next earthquake will occur within `r round(quantile(y_sim, 0.95)
, 2)` hours (`r round(60 * quantile(y_sim, 0.95)
, 0)` minutes)
Roughly, for 95% of earthquakes the waiting time for the next earthquake is less than `r round(quantile(y_sim, 0.95), 2)` hours.
...
See JAGS code below.  The results are very similar to the theoretical results from the previous example.
...
We'll try a Normal prior with the same mean (4/3) and SD (2/3) as a Gamma(4, 3) distribution.
Remember, in JAGS it's `dnorm(mean, precision)`.
The posterior is similar to what we found above; changing from a Normal to Gamma shape didn't change much in this situation.
(Though in other situations it might, and if we had changed prior mean and SD we might also see some changes.)
```{r}
library(rjags)
...
model_string <- "model{
  ...
  # Prior
  theta ~ dgamma(prior_mean, 1 / prior_sd ^ 2)
  prior_mean <- 4 / 3 
  prior_sd <- 2 / 3
