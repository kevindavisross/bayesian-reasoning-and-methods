...
theta_sim = rnorm(n_sim, 98.6, 2)

sigma = 1

y_sim = rnorm(n_sim, theta_sim, sigma)

hist(y_sim)

```

Way too much probability in the tales.
For example, according to this model, `r 100*round(sum(y_sim < 95) / n_sim, 3)` percent of *healthy* humans have hypothermia.
...
theta_sim = rnorm(n_sim, 98.6, 0.3)
...
Provides a more reasonable range of body temperatures.
Still maybe a little too heavy in the tails, but it's certainly an improvement.
...
The posterior distribution of $\theta$ is Normal with posterior mean `r round(mu_n, 1)` and posterior SD `r round(tau_n, 3)` degrees F.


```{r}
theta = seq(97, 100, 0.0001) # the grid is just for plotting
...
We see that the prior has a lot less influence with the sample of size 208 than with the samples size 2.
Also, we see that the likelihood zeros out a lot of the values that we initially gave high prior plausibility to.

```{r}
qnorm(c(0.25, 0.75), mu_n, tau_n)

qnorm(c(0.10, 0.90), mu_n, tau_n)

qnorm(c(0.01, 0.99), mu_n, tau_n)
```


- There is posterior probability of 50% that population mean healthy human body temperature is between `r round(qnorm(0.25, mu_n, tau_n), 2)` and `r round(qnorm(0.75, mu_n, tau_n), 2)` degrees F.
It is equally plausible that mean healthy human body temperature is inside this interval as outside.
- There is posterior probability of 90% that population mean healthy human body temperature is between `r round(qnorm(0.10, mu_n, tau_n), 2)` and `r round(qnorm(0.90, mu_n, tau_n), 2)` degrees F.
It is four times more plausible that mean healthy human body temperature is inside this interval than outside.
- There is posterior probability of 98% that population mean healthy human body temperature is between `r round(qnorm(0.01, mu_n, tau_n), 2)` and `r round(qnorm(0.99, mu_n, tau_n), 2)` degrees F.
It is 49 times more plausible that mean healthy human body temperature is inside this interval than outside.

There is basically 100% posterior probability that mean human body temperatures is less than 98.6 degrees F.
We would be willing to bet almost any amount of money on this.

Of course, all of our conclusions assume that the sample is reasonably representative of healthy human body temperatures.
...
(Old note: The posterior distribution of $\theta$ below is based on the sample of size 2.
The posterior distribution based on the sample of size 2 does *not* seem to provide an adequate model.)

**The posterior is now based on the full sample of size 208.**
...
Simulated samples match up with observed data fairly well.

```{r}

n_sim = 10000

theta_sim = rnorm(n_sim, mu_n, tau_n)
...
There is posterior predictive probability of 95% that a randomly selected healthy adult human body temperature is between `r round(quantile(y_sim, 0.025), 1)` and `r round(quantile(y_sim, 0.975), 1)` degrees F.
Roughly, we predict that 95% of healthy human adults have body temperatures between `r round(quantile(y_sim, 0.025), 1)` and `r round(quantile(y_sim, 0.975), 1)` degrees F.
...
We'll try a prior distribution which places most of its probability on mean birthweight being between 7 and 8 pounds: prior mean of 3400 grams (around 7.5 pounds) and a prior SD of 100 grams (about 0.22 pounds). 
...
Here is the prior predictive distribution.

```{r}

n_sim = 10000

theta_sim = rnorm(n_sim, mu0, tau0)
...
Prior model predicts that about 95% of babies weigh between about 5 and 10 pounds, which seems fairly reasonable.
It's not obviously wrong (no negative birthweights, no 25 pounds babies).
...
The posterior distribution of mean birthweight $\theta$ is Normal with posterior mean `r round(mu_n, 1)` and posterior SD `r round(tau_n, 3)` grams.


```{r}
theta = seq(3200, 3600, 0.01) # the grid is just for plotting
...
- There is posterior probability of 50% that population mean birthweight is between `r round(qnorm(0.25, mu_n, tau_n), 0)` and `r round(qnorm(0.75, mu_n, tau_n), 0)` grams.
It is equally plausible that mean birthweight is inside this interval as outside.
- There is posterior probability of 90% that population birthweight is between `r round(qnorm(0.10, mu_n, tau_n), 0)` and `r round(qnorm(0.90, mu_n, tau_n), 0)` grams.
It is four times more plausible that mean birthweight is inside this interval than outside.
- There is posterior probability of 98% that population mean birthweight is between `r round(qnorm(0.01, mu_n, tau_n), 0)` and `r round(qnorm(0.99, mu_n, tau_n), 0)` grams.
It is 49 times more plausible that mean birthweight is inside this interval than outside.
ey on this.

Of course, all of our conclusions assume that the sample is reasonably representative and the model is correct - but see posterior predictive checking below.
...
There is posterior predictive probability of 95% that a randomly selected newborn baby weighs between `r round(quantile(y_sim, 0.025), 1)` and `r round(quantile(y_sim, 0.975), 1)` grams.
Roughly, we predict that 95% of babies have birthweights between `r round(quantile(y_sim, 0.025), 1)` and `r round(quantile(y_sim, 0.975), 1)` grams.
...
The histogram indicates that the model isn't the greatest.
For example, consider the 99% prediction interval.
We predict that only 1% of birthweights would be outside this interval, but in the sample
`r 100*round((sum(y < quantile(y_sim, 0.005)) + sum(y > quantile(y_sim, 0.995))) / n, 3)` percent of birthweights were outside the interval.
While not a large difference in absolute magnitude, we are observing 3 times more values in the tails than we would expect if birthweights followed a Normal distribution.
...
A Normal likelihood doesn't account for the extremes well enough, and we'll see how to deal with this later.
